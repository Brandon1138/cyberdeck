...
| --- | --- | --- | --- | --- | --- |
| 0A | **Landed** | Executable dependency rule, explicit composition root, and documented allowed-import matrix. | `tests/architecture/dependency-rule.test.ts`, `docs/architecture/dependency-rule-baseline.json`, `docs/architecture/dependency-rule.md`. The sole composition importer is the executable bootstrap edge `src/cli.ts -> src/broker/main.ts`. | Low. | Focused architecture test and full suite retain the exact baseline ratchet. |
