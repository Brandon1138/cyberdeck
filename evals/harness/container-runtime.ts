 ...
 */
export async function containerRuntime(root: string, guest: { kind: "scripted" } | { kind: "provider"; live: LiveEvalConfig }, activity?: AgentActivityPort): Promise<ContainerRuntimeFixture> {
  ...
  const provider = guest.kind === "provider" ? guest.live.provider : "claude";
  ...
  const runtime = await brokerExecutionRuntime({ stateDirectory: state, config, adapters: hostAdapters, lookupSession: (id) => lookup(id), submitEvent: (input) => submit(input), ...(activity ? { activity } : {}) });
